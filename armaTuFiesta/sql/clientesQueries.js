...
exports.CREAR_CLIENTES =
  "SET @reside = ?; SET @rol = (SELECT pkRol FROM (SELECT id_rol as pkRol FROM arma_tu_fiesta.rol WHERE nombre = ?) as tablaRol); INSERT INTO arma_tu_fiesta.persona (cedula, p_nombre, s_nombre, p_apellido, s_apellido, fecha_nacimiento, sexo, estado_civil, fk_residencia) VALUES (?,?,?,?,?,?,?,?,@reside); SET @usuario = (SELECT pkPersona FROM (SELECT id_persona as pkPersona from arma_tu_fiesta.persona WHERE cedula = ?) as tablaPersona); INSERT INTO arma_tu_fiesta.telefono (numero, fk_persona) VALUES (?, @usuario); INSERT INTO arma_tu_fiesta.email (correo, fk_persona) VALUES (?, @usuario); INSERT INTO arma_tu_fiesta.usuario (username, password, fk_persona, fk_rol) VALUES (?, ? , @usuario, @rol);"

//   [parroquia, p_nombre, cedula, p_nombre, s_nombre, p_apellido, s_apellido, fecha_nacimiento, sexo, estado_civil, cedula, numero, correo, correo, password]
...
exports.ACTUALIZAR_CLIENTES =
  "SET @persona = ?; UPDATE arma_tu_fiesta.telefono SET numero = ? WHERE fk_persona = @persona; UPDATE arma_tu_fiesta.email SET correo = ? WHERE fk_persona = @persona; UPDATE arma_tu_fiesta.usuario SET username = ? WHERE fk_persona = @persona; UPDATE arma_tu_fiesta.usuario SET password = ? WHERE fk_persona = @persona;"

  //[id_persona, telefono, correo, correo, password]

exports.ELIMINAR_CLIENTES =
  "SET @persona = ?; DELETE FROM arma_tu_fiesta.usuario WHERE fk_persona = @persona; DELETE FROM arma_tu_fiesta.telefono WHERE fk_persona = @persona; DELETE FROM arma_tu_fiesta.email WHERE fk_persona = @notaripersonaa; DELETE FROM arma_tu_fiesta.persona WHERE id_persona = @persona;"
