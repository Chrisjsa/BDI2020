...
exports.SIGN_UP =
  "SET @fk_residencia =  ?; -- parroquia INSERT INTO persona (cedula, p_nombre, s_nombre, p_apellido, s_apellido, fecha_nacimiento, sexo, estado_civil, fk_residencia) VALUES (?, ?, ?, ?, ?, ?, ?, ?, @fk_residencia); -- cedula, p_nombre, etc SET @fk_persona = (SELECT id_persona FROM persona WHERE cedula = ?); -- cedula SET @fk_rol = (SELECT id_rol FROM rol WHERE nombre = Cliente); -- NO se cambia; INSERT INTO usuario (username, password, fk_persona, fk_rol) VALUES  (?, ?, @fk_persona, @fk_rol); -- username, password SET @fk_residencia = NULL; SET @fk_persona = NULL; SET @fk_rol = NULL;"

//[parroquia, cedula, p_nombre, s_nombre, p_apellido, s_apellido, fecha_nacimiento, sexo, estado_civil, username, password]
