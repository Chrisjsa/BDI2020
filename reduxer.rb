...
"export const %s = (%s) => async dispatch => { setLoading()(dispatch); try { const res = await axios.%s(`api/%s`, %s, config); dispatch({ type: %s, payload: res.data, }) } catch (error) { dispatch({ type: ERROR, payload: error.response.msg, }) } }" % [
    fc, args, html_request, api_route, args, type_format
])
